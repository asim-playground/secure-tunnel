    ...
    static func main() throws {
        ...
        let auth = try connection.authenticateAccount(
            request: AccountAuthRequest(
                accountId: "swift-package-smoke",
                credentialPayload: Data("credential".utf8),
                mode: .fresh
            )
        )
        guard auth.accountId == "swift-package-smoke" else {
        ...
        print(
            #"{"language":"swift","package":"SecureTunnel","protocol":"\#(protocolIdV1())","carrier":"quic","close":"\#(close.classification)"}"#
        )
    }
}
