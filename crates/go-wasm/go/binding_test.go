...
func TestParseError(t *testing.T) {
	...
	if err.Error() == "Error: expected a number between '+' operators" {
		t.Fatal("Expected transport prefix to be stripped from WASM parse errors")
	}
}
...
func TestParseOverflow(t *testing.T) {
	ctx := context.Background()
	_, err := Parse(ctx, "9223372036854775807+1")
